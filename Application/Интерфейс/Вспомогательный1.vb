...
Module Вспомогательный
    ...
    Function активироватьМодули(Форма As Form, Программа As String) As Object
        ...
        СтрокаЗапроса = "SELECT Модуль, Часы, НомерМодуля FROM ПрограммаМодулиЧасы WHERE Программа = " & Chr(39) & Программа & Chr(39) & " ORDER BY ПрограммаМодулиЧасы.НомерМодуля"
        Модули = ЗагрузитьИзБазы.ЗагрузитьИзБазы(СтрокаЗапроса)
        счетчикСтрок = UBound(Модули, 2) + 1
    ...
    End Function
    Sub ОткрытьФорму(Form As Object)
    ...
    Function ПутьКОбщейБазеИЗTXT(ПутьКФайлуRes As String) As String
        Dim счетчик As Integer
Чтение:
        Try
            АдрессОбщейБазы = File.ReadAllText(ПутьКФайлуRes & "АдресБД.txt")
        Catch ex As Exception

            ААОсновная.СписокОшибок.Text += "Ошибка при запуске! ex.HResult =" & ex.HResult & " Текст:" & ex.Message + Environment.NewLine

            Call Интерфейс.ЗаписьОшибокВТХТ("Процедура Запуск приложения", ex)
        ...
        End Try
        ПутьКОбщейБазеИЗTXT = АдрессОбщейБазы
    End Function

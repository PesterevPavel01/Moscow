Module СправкаОбОкончании
    ...
    Sub СправкаОбОкончании(ЧекнутыеСлушатели As Object)
        Dim ПриложениеВорд
        Dim ДокументВорд, Модули, Группа, Таблица
        ...
        СтрокаЗапроса = selectSpravkaIA_group(ААОсновная.prikazKodGroup)
        ...
        СтрокаЗапроса = selectSpravka_moduls_hours(ААОсновная.prikazKodGroup)
        Модули = ЗагрузитьИзБазы.ЗагрузитьИзБазы(СтрокаЗапроса)

        If Модули(0, 0) = "нет записей" Then
        ...
        ПутьКШаблону = ПутьККаталогуСРесурсами & "Шаблоны\Справка об окончании без ИА.docx"

        ПриложениеВорд = CreateObject("Word.Application")

        For СчетчикСтрок = 0 To UBound(ЧекнутыеСлушатели, 2)

            ВидПриказа = "Справка об окончании без ИА" & ЧекнутыеСлушатели(0, СчетчикСтрок)
...
" & АСформироватьПриказ.path
            ...
            'If Вспомогательный.СоздатьПапку(ПутьККаталогуСРесурсами & "Группы\Группа N" & АСформироватьПриказ.НомерГруппы.Text & "\Справки") Then
            '    Вспомогательный.сохранить(ДокументВорд, ВидПриказа, ПутьККаталогуСРесурсами & "Группы\Группа N" & АСформироватьПриказ.НомерГруппы.Text & "\Справки\")
            '    предупреждение.текст.Text = "Справки сохранены, Путь к каталогу:" & ПутьККаталогуСРесурсами & "Группы\Группа N" & АСформироватьПриказ.НомерГруппы.Text & "\Справки"
            'Else
            '    Вспомогательный.сохранить(ДокументВорд, ВидПриказа, ПутьККаталогуСРесурсами & "Группы\Нераспределенное\")
            '    предупреждение.текст.Text = "Справки сохранены, Путь к каталогу:" & ПутьККаталогуСРесурсами & "Группы\Нераспределенное"
            'End If
            ...
            Вспомогательный.ЗаменитьТекстВОбластиДокументаВорд(ДокументВорд.Range, "$ГруппаЧасы$", Группа(5, 0), 2)
            ...
            Таблица = НайтиТаблицуПоМеткеИлиНеНайдена(ДокументВорд.Range, "$Таблица$", 2, 2)
            ...
            МСВорд.ЗаполнитьТаблицу(Таблица, Модули, 2, False)

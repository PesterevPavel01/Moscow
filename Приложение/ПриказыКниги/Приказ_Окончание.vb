...
Module Приказ_Окончание
    ...
    Sub Приказ_Окончание(ВидПриказа)
        ...
        If ВидПриказа = "ПК_Окончание" Then
            ...
            СтрокаЗапроса = "SELECT CONCAT(Слушатель.Фамилия,' ',Слушатель.Имя,' ',IFNULL(слушатель.Отчество,' ')), 'зачтено'  FROM СоставГрупп INNER JOIN Слушатель ON СоставГрупп.Слушатель = Слушатель.Снилс WHERE СоставГрупп.Kod = " & ААОсновная.prikazKodGroup & " ORDER BY Слушатель.Фамилия"
    ...
    Sub ПрисвоитьНомера(Параметры As Object)
        ...
        While Счетчик <= UBound(Группа, 2)
            ...
            If Параметры(0) = "Удостоверение" Then
                строкаЗапроса = "UPDATE СоставГрупп SET НомерУд = " & Группа(4, 0) + Счетчик & " , РегНомерУд= " & Группа(5, 0) + Счетчик & ", НомерСвид=0, РегНомерСвид=0, НомерДиплома=0, РегНомерДиплома=0  WHERE Kod = " & Параметры(2) & " AND Слушатель = " & Chr(39) & Группа(3, Счетчик) & Chr(39)
            ElseIf Параметры(0) = "Свидетельство" Then
                строкаЗапроса = "UPDATE СоставГрупп SET НомерУд = 0 , РегНомерУд= 0, НомерСвид = " & Группа(4, 0) + Счетчик & " , РегНомерСвид= " & Группа(5, 0) + Счетчик & ", НомерДиплома=0, РегНомерДиплома=0 WHERE Kod = " & Параметры(2) & " AND Слушатель = " & Chr(39) & Группа(3, Счетчик) & Chr(39)
            ElseIf Параметры(0) = "Диплом" Then
                строкаЗапроса = "UPDATE СоставГрупп SET НомерУд = 0 , РегНомерУд= 0,НомерСвид=0, РегНомерСвид=0, НомерДиплома = " & Группа(4, 0) + Счетчик & " , РегНомерДиплома= " & Группа(5, 0) + Счетчик & " WHERE Kod = " & Параметры(2) & " AND Слушатель = " & Chr(39) & Группа(3, Счетчик) & Chr(39)
            End If

...
Module ПК_Окончание
    ...
    Sub ПК_Окончание_уд(ЧекнутыеСлушатели As Object, ВидПриказа As Object)
        ...
        СтрокаЗапроса = "SELECT Слушатель.Фамилия+' '+ Слушатель.Имя+' '+IFNULL(слушатель.Отчество,' '),Слушатель.Фамилия,Слушатель.Имя+' '+IFNULL(слушатель.Отчество,' '),Слушатель.Снилс,Группа.НомерУд, Группа.РегНомерУд,Группа.ОсновнойДокумент,Программа , ДатаВыдачиУд, ДатаНЗ, ДатаКЗ, КолЧас FROM (СоставГрупп INNER JOIN Слушатель ON СоставГрупп.Слушатель = Слушатель.Снилс) INNER JOIN Группа ON СоставГрупп.Kod = Группа.Код WHERE Группа.Код = " & ААОсновная.prikazKodGroup & " ORDER BY Слушатель.Фамилия"
    ...
    Sub ПрисвоитьНомера(parametrs As Object)
        ...
        While Счетчик <= UBound(Группа, 2)
            ...
            строкаЗапроса = "UPDATE СоставГрупп SET НомерУд = " & Группа(4, 0) + Счетчик & " , РегНомерУд= " & Группа(5, 0) + Счетчик & " WHERE Kod = " & kodGroup & " AND Слушатель = " & Chr(39) & Группа(1, Счетчик) & Chr(39)

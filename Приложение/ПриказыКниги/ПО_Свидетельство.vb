Module ПО_Свидетельство
    Sub ПО_Свидетельство(видПриказа As String)
        ...
        СтрокаЗапроса = "SELECT Слушатель.Фамилия,Слушатель.Имя,IFNULL(слушатель.Отчество,' '), Слушатель.ДатаРождения,ОценкаМодуль1,ОценкаМодуль2,ОценкаМодуль3,ОценкаМодуль4,ОценкаМодуль5,ОценкаМодуль6,ОценкаМодуль7,ОценкаМодуль8,ОценкаМодуль9,ОценкаМодуль10,ИАПрактическиеНавыки,ИАИтог, СоставГрупп.РегНомерСвид, Группа.ДатаВыдачиСвид, Группа.ДатаКЗ,НаимДОО, Квалификация,КолЧас,ДатаНЗ, Программа FROM (СоставГрупп INNER JOIN Слушатель ON СоставГрупп.Слушатель = Слушатель.Снилс) INNER JOIN Группа ON СоставГрупп.Kod = Группа.Код WHERE Группа.Код =" & ААОсновная.prikazKodGroup & " ORDER BY Слушатель.Фамилия"

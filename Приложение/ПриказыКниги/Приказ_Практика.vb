Module Приказ_Практика
    Sub Приказ_Практика(ВидПриказа As String)
        ...
        СтрокаЗапроса = "SELECT CONCAT(Слушатель.Фамилия,' ',Слушатель.Имя,' ',IFNULL(слушатель.Отчество,' ')), Группа.Программа, ДатаНЗ, ДатаКЗ, КолЧас FROM (СоставГрупп INNER JOIN Слушатель ON СоставГрупп.Слушатель = Слушатель.Снилс) INNER JOIN Группа ON СоставГрупп.Kod = Группа.Код WHERE Группа.Код = " & ААОсновная.prikazKodGroup & " ORDER BY Слушатель.Фамилия"

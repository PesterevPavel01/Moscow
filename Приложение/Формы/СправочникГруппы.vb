Public Class СправочникГруппы
    ...
    Sub ПоискПоСтрокеПоиска()
        ...
        Else
            ...
            massiv = Поиск.SQLПоиск(СтрокаПоиска.Text, "Группа", "Код, Номер, Программа, Куратор, ДатаНЗ, ДатаКЗ", ПолеДляПоиска, ПолеДляСортировки & Интерфейс.ВидСортировки(Me, НастройкаСортировкиГрупп.НажатПБГр))
    ...
    Private Sub ListViewСписокГрупп_KeyDown(sender As Object, e As KeyEventArgs) Handles ListViewСписокГрупп.KeyDown
        ...
        If e.KeyCode = 46 Then
            ...
            If Not ЗаписьВБазу.ПроверкаСовпаденийЧислоДА_2("Группа", "Код", kod) = 2 Then
            ...
            'ФормаДаНетУдалить.Label1.Text = "Группа № " & ind
            ...
            If ФормаДаНетУдалить.НажатаКнопкаДа Then
                ...
                If ЗаписьВБазу.ПроверкаСовпаденийЧислоДА_2("СоставГрупп", "Kod", kod) = 2 Then
                    ...
                    ЗаписьВБазу.УдалитьЗаписиСЧислом("СоставГрупп", "Kod", kod)

                End If

                ЗаписьВБазу.УдалитьЗаписиСЧислом("Группа", "Код", kod)
...
Конец:
    ...
    Private Sub ListViewСписокГрупп_DoubleClick(sender As Object, e As EventArgs) Handles ListViewСписокГрупп.DoubleClick
        ...
        If Not ListViewСписокГрупп.SelectedItems.Item(0).SubItems(1).Text = "удалено" Then
            ...
            СтрокаЗапроса = "SELECT Код FROM Группа WHERE Код = " & kod

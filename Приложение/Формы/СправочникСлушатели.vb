Public Class ФормаСправочникСлушатели
    ...
    Sub ПоискПоСтрокеПоиска()
        ...
        If ПолеДляПоиска = "Снилс" Then
            ...
            massiv = Поиск.SQLПоиск(ДобавитьРубашку.УдалитьРубашку(СтрокаПоиска.Text), "Слушатель", "Снилс, Фамилия, Имя, Отчество, ДатаРождения", ПолеДляПоиска, ПолеДляСортировки & Интерфейс.ВидСортировки(НастройкаСортировкиСлушателей, НастройкаСортировкиСлушателей.НажатПБСл))
        ...
        Else
            ...
            massiv = Поиск.SQLПоиск(СтрокаПоиска.Text, "Слушатель", "Снилс, Фамилия, Имя, Отчество, ДатаРождения", ПолеДляПоиска, ПолеДляСортировки)
            massiv = ДобавитьРубашкуВМассив(massiv, 0)
    ...
    Private Sub ListViewСписокСлушателей_KeyDown(sender As Object, e As KeyEventArgs) Handles ListViewСписокСлушателей.KeyDown
        ...
        If e.KeyCode = Keys.Delete Then
            ...
            If ФормаДаНетУдалить.НажатаКнопкаДа Then
                ...
                If ЗаписьВБазу.ПроверкаСовпадений("СоставГрупп", "Слушатель", ind) Then

                    ЗаписьВБазу.УдалитьЗаписи("СоставГрупп", "Слушатель", ind)

...
Public Class СписокСлушателейВГруппе
    ...
    Sub СписокСлушВГруппе(argument)
        ...
        queryStr = "SELECT Слушатель.Снилс, Слушатель.Фамилия, Слушатель.Имя, IFNULL(слушатель.Отчество,' '), Слушатель.ДатаРождения FROM СоставГрупп INNER JOIN Слушатель ON СоставГрупп.Слушатель = Слушатель.Снилс WHERE СоставГрупп.Kod = " & argument(0) & " ORDER BY Слушатель.Фамилия"
    ...
    Private Sub ListViewСписокСлушателей_KeyDown(sender As Object, e As KeyEventArgs) Handles ListViewСписокСлушателей.KeyDown
        ...
        If e.KeyCode = 46 Then
            ...
            If ЗаписьВБазу.ПроверкаСовпаденийЧислоДА_2("СоставГрупп", "Kod", СправочникГруппы.kod, "Слушатель", Снилс) = 2 Then
                ЗаписьВБазу.УдалитьЗаписиСЧислом("СоставГрупп", "Kod", СправочникГруппы.kod, "Слушатель", Снилс)

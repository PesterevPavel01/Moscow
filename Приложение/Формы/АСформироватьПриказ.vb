Public Class АСформироватьПриказ
    ...
    Private Sub Сформировать_Click(sender As Object, e As EventArgs) Handles КнопкаСформировать.Click
        ...
        Else
            АПриказ.ПриказОЗачислении(ВидПриказа)
        End If
    ...
    Sub ЗагрузитьСписокСлушателей()
        ...
        СтрокаЗапроса = "SELECT CONCAT(Слушатель.Фамилия,' ',Слушатель.Имя,' ',IFNULL(слушатель.Отчество,' ')) FROM СоставГрупп INNER JOIN Слушатель ON СоставГрупп.Слушатель = Слушатель.Снилс WHERE СоставГрупп.Kod = " & ААОсновная.prikazKodGroup & " ORDER BY Слушатель.Фамилия"

...
Public Class DGVRedactorTable
    ...
    Public DataGridTablesResult As DataGridView
    Dim sqlQuery As String

    Public Sub removeRow()

        Dim curNumber = DataGridTablesResult.CurrentCell.RowIndex
        ФормаДаНетУдалить.текстДаНет.Text = "Удалить " + DataGridTablesResult.Rows(curNumber).Cells(userDGV.numberElementFirst).Value + "?"
        ...
        userDGV.remove_kod = Convert.ToInt64(DataGridTablesResult.Rows(curNumber).Cells(userDGV.kod_number).Value)

        If userDGV.program_on Then
            sqlQuery = MainForm.sqlQueryString.programs__checkKodGrouppBusy(userDGV.remove_kod)
            userDGV.result = userDGV.mySQLConnector.loadIntoArray(sqlQuery, 1, 0)

            If Convert.ToInt16(userDGV.result(0)) > 0 Then
        ...
        If userDGV.redactor_element_first.Text.Trim = Convert.ToString(DataGridTablesResult.Rows(curNumber).Cells(userDGV.numberElementFirst).Value) Then
        ...
        If (curNumber <> 0) Then
            ...
            DataGridTablesResult.CurrentCell = DataGridTablesResult.Rows(curNumber - 1).Cells(userDGV.numberElementFirst)

...
Module Запуск
    ...
    End Function
    Sub УвеличитьПрогресс(параметр As Integer)

        Загрузка.Выполнение.Value = параметр

        If параметр = 100 Then
            Загрузка.Hide()
        End If

    End Sub
    Sub ЗакрытьЗагрузку(параметр As String)

        Загрузка.Hide()

    End Sub

    Sub СохранениеНастройки(Параметр As String, Значение As String)

        Dim СтрокаЗапроса As String
        Dim МассивДляЗаписиВОбщуюБазу

        ReDim МассивДляЗаписиВОбщуюБазу(1)
        СтрокаЗапроса = "DELETE * FROM Настройки WHERE Параметр=" & Chr(39) & Параметр & Chr(39)
        МассивДляЗаписиВОбщуюБазу(0) = СтрокаЗапроса
        СтрокаЗапроса = "INSERT INTO Настройки (Параметр, Значение) VALUES (" & Chr(39) & Параметр & Chr(39) & " , " & Chr(39) & Значение & Chr(39) & ")"
        МассивДляЗаписиВОбщуюБазу(1) = СтрокаЗапроса

    End Sub
    Sub ЗакрытьФормуЗавершениеРаботы(Параметр As Integer)

        ЗавершениеРаботы.Close()
